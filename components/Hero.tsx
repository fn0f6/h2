...
const Hero: React.FC = () => {
  ...
  return (
    <section id="hero" className="relative min-h-[90vh] md:min-h-[95vh] flex items-center pt-24 overflow-hidden">
      <div 
        className="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-[20s] scale-110 animate-fade-in will-change-transform"
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 w-full relative z-10 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="space-y-8 md:space-y-10 text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-display font-black text-white leading-[1.1] md:leading-[0.95] tracking-tighter opacity-0 animate-fade-in-up delay-100 will-change-transform">
            {t.heroHeadline.split(' ').map((word: string, i: number) => (
               <React.Fragment key={i}>
                 <span className={i >= t.heroHeadline.split(' ').length - 2 ? "text-gold text-glow" : "text-white"}>
                   {word}{' '}
                 </span>
               </React.Fragment>
            ))}
           ...
           <div className="animate-float will-change-transform">
             <div className="relative p-1 bg-gradient-to-tr from-gold/40 to-transparent rounded-[3rem] shadow-2xl overflow-hidden">
                <img 
                  ...
                  alt="" 
                  loading="eager"
                  // Fix: Use camelCase fetchPriority instead of fetchpriority to match React's JSX props
                  fetchPriority="high"
